...
y_dim = 10
t = np.arange(-0.625*40,0.65625*40,0.03125*40)
batch_orig = 32
stepUse =34000
...
sampler_c = Sample_c()    
num_c_samp = 16    
imgOrig = np.zeros((y_dim,28,28,1))

z_pt_samp = np.zeros((y_dim,num_c_samp,z_dim))
img_samp = np.zeros((y_dim,num_c_samp,28,28,1))
for k in range(0,10):
    class_num = k
    idxClass = np.where(data_y[:,class_num] == 1)[0]
    numEx = len(idxClass)

    idxChoice = idxClass[10]
    imgInput = np.expand_dims(data_X[idxChoice,:,:,:],axis=0)
    input_temp = imgInput
    ...
    for m in range(0,num_c_samp):
        z_coeff = sampler_c(1,M,opt.post_l1_weight*0.23)
        z_scale_samp = transNet(test_latent_temp_scale.double(),z_coeff.double(),Psi,opt.to_noise_std*5)
