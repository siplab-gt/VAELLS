...
*Figure 2 and 3* (swiss roll experimental results):
 - To train VAELLS: VAELLS.py with data_type: 'swiss2D'
 - To make data for plots: run `TOVAE_swissRoll2D_dict_createDataPlots.py`. 
 - To create plots: run 'plotSwissRollOutputs.m' 
 
*Figure 4, 7, 8, 9* (concentric circle experimental results):
 - To train VAELLS: VAELLS.py with data_type: 'concen_circle'
 - To make data for plots: run `TOVAE_circle_dict_createDataPlots.py`. 
 - To create plots: run 'plotCircleOutputs.m' 

*Figure 5* (rotated MNIST experimental results)
 - To train VAELLS: VAELLS.py with data_type: 'rotDigits'
 - To make data for plots: run `TOVAE_dict_rotDigits_genTransOptSeq.py`. 
 - To create plots: run 'plotTransOptImgOrbits_TOVAE_rotDigits.m' 

*Figure 6* (natural MNIST experimental results)
 - To train VAELLS: VAELLS.py with data_type: 'natDigits'
 - To make data for plots: run `TOVAE_dict_natDigits_genTransOptSeq.py`. 
